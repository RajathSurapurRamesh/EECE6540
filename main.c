...
static float A[200*400];

static float B[400*600];

static float C[200*600];
...
#error "unknown OpenCL SDK environment"
    ...
    cl_mem bufferTmp = clCreateBuffer(context, CL_MEM_READ_WRITE,
	   wTmp*hTmp*sizeof(float), NULL, &ret);
    clEnqueueWriteBuffer(command_queue, bufferTmp, CL_TRUE, 0,
            wTmp*hTmp*sizeof(float), (void *)Tmp, 0, NULL, NULL);
    ...
    clSetKernelArg(kernel, 7, sizeof(cl_int), (void *)&wTmp);
    clSetKernelArg(kernel, 8, sizeof(cl_int), (void *)&hTmp);
    clSetKernelArg(kernel, 9, sizeof(cl_mem), (void *)&bufferA);
    clSetKernelArg(kernel,10, sizeof(cl_mem), (void *)&bufferB);
    clSetKernelArg(kernel,11, sizeof(cl_mem), (void *)&bufferC);
    clSetKernelArg(kernel,12, sizeof(cl_mem), (void *)&bufferTmp);
   ...
   clEnqueueReadBuffer(command_queue, bufferTmp, CL_TRUE, 0, wTmp*hTmp*sizeof(float),
         (void *)Tmp, 0, NULL, NULL);
